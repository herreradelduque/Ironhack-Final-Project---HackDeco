    ...
    for i, shop in enumerate(shop_lst):
        for j, product in enumerate(product_lst):
            ...
            for f in files:
                filename = f
                original = load_img(
                    filename, target_size=(imgs_model_width, imgs_model_height)
                )
            ...
            np.save(
                f"../data/features_extraction_products/img_features_{shop_lst[i]}_{product_lst[j]}",
                imgs_features,
            )
            print(
                f"img_features_user saved at: '../data/features_extraction_products/img_features_{shop_lst[i]}_{product_lst[j]}'"
            )
