...
def ikea_scrapper():
    ...
    for index, product in enumerate(product_lst):
        ...
        for link in soup.find_all("img class"):
            src = link.get("src")
            if src.startswith(
                "https://www.ikea.com/es/es/images/products/"
            ):  # check src starts with...
        ...
        product_price_list = []
        prices = soup.select(
            "div.product-compact__spacer span.product-compact__price__value"
        )
        for price in prices:
        ...
        # Create DataFrame:
        scraped_dict = {
            "Type": product_types_list,
            "Name": product_names_list,
            "Price": product_price_list,
            "Image_url": image_list,
            "Buy_url": buy_url_list_unique,
            "Local_Path": images_path_list,
        }
